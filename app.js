// State Management
let state = {
    currentSection: 'home', // home, dashboard, reader, quiz, search, cheat-sheet, audit, phishing, notes
    currentChapterNum: 0,
    theme: 'dark',
    progress: {},
    fontSize: 16,
...
function renderSidebarNav() {
    ...
    const mainLinks = [
        { id: 'home', label: 'Home Gate', icon: `<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path><polyline points="9 22 9 12 15 12 15 22"></polyline></svg>` },
...
// Matrix Falling Node Rain Background Canvas
function initMatrixCanvas() {
    ...
    const columnsCount = Math.floor(width / 50) + 1;
    const streams = [];
    ...
    for (let i = 0; i < columnsCount; i++) {
        streams.push({
            ...
            nodeCount: 4 + Math.floor(Math.random() * 6),
            binaryStrings: Array.from({length: 8}, () => {
                const rand = Math.random();
                if (rand < 0.3) return '01';
                if (rand < 0.6) return '110';
                if (rand < 0.85) return '0100000';
                return '1101';
            }),
            glowNodeIdx: Math.floor(Math.random() * 8)
        });
    }
    ...
    function draw() {
        ...
        streams.forEach(stream => {
            // Draw a subtle vertical stream trace line
            ctx.beginPath();
            ctx.strokeStyle = 'rgba(0, 255, 102, 0.05)';
            ctx.lineWidth = 1;
            ctx.moveTo(stream.x, stream.y);
            ctx.lineTo(stream.x, stream.y + 300);
            ctx.stroke();
            
            // Draw binary blocks and glowing nodes
            for (let j = 0; j < 8; j++) {
                const itemY = stream.y + j * 35;
                if (itemY > -20 && itemY < height + 20) {
                    if (j === stream.glowNodeIdx) {
                        // Drawing glowing data node dots like reference image
                        ctx.beginPath();
                        ctx.arc(stream.x, itemY, 5, 0, Math.PI * 2);
                        ctx.fillStyle = '#00ff66';
                        ctx.shadowColor = '#00ff66';
                        ctx.shadowBlur = 12;
                        ctx.fill();
                        ctx.shadowBlur = 0; // reset glow immediately
                    } else {
                        // Digital characters flowing down
                        ctx.font = '11px monospace';
                        ctx.fillStyle = 'rgba(0, 255, 102, 0.22)';
                        ctx.fillText(stream.binaryStrings[j], stream.x - 6, itemY);
                    }
                }
            }
            ...
            if (stream.y > height) {
                stream.y = Math.random() * -300 - 100;
                stream.speed = 1.2 + Math.random() * 2;
                stream.glowNodeIdx = Math.floor(Math.random() * 8);
            }
        });
...
// Update stats readout on the Home Gate page
function updateHomeStatsUI() {
    const bookmarkCountEl = document.getElementById('home-bookmark-count');
    const progressRatingEl = document.getElementById('home-progress-rating');
    const lastChapterEl = document.getElementById('home-last-chapter');
    
    if (bookmarkCountEl) {
        bookmarkCountEl.innerText = state.bookmarks.length;
    }
    
    const totalChapters = state.toc.length || 21;
    const readChapters = Object.keys(state.progress).filter(k => state.progress[k] === true).length;
    const percent = Math.round((readChapters / totalChapters) * 100);
    
    if (progressRatingEl) {
        progressRatingEl.innerText = `${percent}%`;
    }
    
    if (lastChapterEl) {
        const readChapterNums = Object.keys(state.progress)
            .filter(k => state.progress[k] === true)
            .map(Number)
            .sort((a, b) => b - a); // sort descending
            
        if (readChapterNums.length > 0) {
            const lastNum = readChapterNums[0];
            const chObj = state.toc.find(c => c.num === lastNum);
            if (chObj) {
                lastChapterEl.innerText = `Ch ${lastNum}: ${chObj.title.substring(0, 15)}...`;
            } else {
                lastChapterEl.innerText = `Chapter ${lastNum}`;
            }
        } else {
            lastChapterEl.innerText = 'None';
        }
    }
}
